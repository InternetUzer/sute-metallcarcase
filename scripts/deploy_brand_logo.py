...
def replace_file(source, target):
    ...
    descriptor, name = tempfile.mkstemp(prefix='.brand-logo-', dir=target.parent)
    os.close(descriptor)
    temporary = Path(name)
    try:
...
def main():
    ...
    manifest = json.loads((source / 'docs/brand-logo-manifest.json').read_text())
    ...
    backup = Path(tempfile.mkdtemp(prefix='metalcarcase-brand-logo-' + stamp + '-', dir=production.parent))
    backup.chmod(0o700)
...
'''
    ...
    try:
        ...
        (production / 'instance/brand-logo-release.json').write_text(json.dumps({'backup': str(backup), 'installed_at': stamp,
