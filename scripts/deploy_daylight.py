...
def replace_file(source, target):
    ...
    descriptor, name = tempfile.mkstemp(prefix='.daylight-', dir=target.parent)
    os.close(descriptor)
    temporary = Path(name)
    try:
...
def main():
    ...
    manifest = json.loads((source / 'docs/daylight-manifest.json').read_text())
    ...
    backup = Path(tempfile.mkdtemp(prefix='metalcarcase-daylight-' + stamp + '-', dir=production.parent))
    backup.chmod(0o700)
...
'''
    ...
    try:
        ...
        (production / 'instance/daylight-release.json').write_text(json.dumps({'backup': str(backup), 'installed_at': stamp,
