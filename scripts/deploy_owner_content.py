...
def replace_file(source, target):
    ...
    descriptor, name = tempfile.mkstemp(prefix='.owner-content-', dir=target.parent)
    os.close(descriptor)
    temporary = Path(name)
    try:
...
def main():
    ...
    manifest = json.loads((source / 'docs/owner-content-manifest.json').read_text())
    ...
    backup = Path(tempfile.mkdtemp(prefix='metalcarcase-owner-content-' + stamp + '-', dir=production.parent))
    backup.chmod(0o700)
...
for path in ['/', '/uslugi', '/request', '/produkciya', '/sitemap.xml', '/betonnye-raboty', '/krovelnye-raboty', '/o-kompanii', '/kontakty', '/robots.txt']:
...
for path in ['/admin','/admin/analytics','/admin/cases','/admin/photos','/admin/seo','/admin/services','/admin/services/1','/admin/services/1/preview','/admin/content','/admin/content/sections/home','/admin/content/sections/home/preview','/admin/content/sections/company','/admin/content/sections/contacts','/admin/content/sections/enquiry']:
    response=client.get(path,base_url=base)
    assert response.status_code==200, (path,response.status_code)
print('Isolated database, owner editors and public route checks passed.')
'''
    ...
    try:
        ...
        (production / 'instance/owner-content-release.json').write_text(json.dumps({'backup': str(backup), 'installed_at': stamp,
