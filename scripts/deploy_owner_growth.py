...
def replace_file(source, target):
    ...
    descriptor, name = tempfile.mkstemp(prefix='.owner-growth-', dir=target.parent)
    os.close(descriptor)
    temporary = Path(name)
    try:
...
def main():
    ...
    manifest = json.loads((source / 'docs/owner-growth-manifest.json').read_text())
    ...
    backup = Path(tempfile.mkdtemp(prefix='metalcarcase-owner-growth-' + stamp + '-', dir=production.parent))
    backup.chmod(0o700)
...
for path in ['/', '/request', '/produkciya', '/sitemap.xml']:
...
for path in ['/admin','/admin/analytics','/admin/cases','/admin/photos','/admin/seo']:
    response=client.get(path,base_url=base)
    assert response.status_code==200, (path,response.status_code)
print('Isolated database and route checks passed.')
'''
    ...
    try:
        ...
        (production / 'instance/owner-growth-release.json').write_text(json.dumps({'backup': str(backup), 'installed_at': stamp,
